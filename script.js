...
async function loadAdminSettings() {
  if (isSavingSettings) return; // Skip polling while we are saving to avoid race conditions
...
async function saveAdminSettings() {
  ...
  try {
    ...
    if (r.ok) {
      ...
      console.error("Failed to save settings. Status:", r.status);
    }
  } catch (e) {
...
async function saveStudent(record) {
  try {
    ...
    const j = await r.json();
    if (j.success) {
      // If we're on admin page, refresh immediately
      if (document.getElementById("page5")) showMessList(true);
      return true;
    } else {
      alert(j.message || "Submission failed.");
      return false;
    }
  } catch (e) {
    console.error("Failed to submit record to API:", e);
